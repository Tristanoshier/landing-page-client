          ...
          <div className={props.darkMode ? "card" : "card light"} key={index}>
            <div
              className={
                props.darkMode
                  ...
                  : "interest-card-header light"
              }
            >
              <span className="top-row">
                <p
                  className={props.darkMode ? "card-title" : "card-title light"}
                >
                  {project.title}
                </p>
                <p className={props.darkMode ? "card-date" : "card-date light"}>
                  {convertDate(project.createdAt)}
                </p>
              </span>
              <span
                className={props.darkMode ? "card-topic" : "card-topic light"}
              >
                {project.languages}
              </span>
            </div>
            {project.description.length > 299 ? (
              <div className="card-body">
                <div
                  className={props.darkMode ? "card-text" : "card-text light"}
                  dangerouslySetInnerHTML={{
                    __html: `${project.description.substr(0, 299)}. . .`,
                  }}
                />
                <br />
                <Link
                  onClick={() => saveProjectInLocalStorage(project)}
                  className={props.darkMode ? "card-link" : "card-link light"}
                  to={{
                    pathname: `/FocusedProject/${project.title}`,
                    project: project,
                  }}
                >
                  See More
    ...
    <div>
      <div className="blog">
        ...
        <p className={props.colorMode("card-text", "card-text light")}>
            Mostly projects from my early days of learning how to code or projects that I built with friends :)
        </p>
        {isLoading ? <div className="loader"></div> : projectMapper()}
